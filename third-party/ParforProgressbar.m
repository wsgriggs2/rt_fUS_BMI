...
%
% September 2023 - Modified to suppress udp deprecation warning on newer MATLAB
%                  versions. WG
    ...
    methods
        function o = ParforProgressbar( numIterations, varargin )
            ...
            if iscell(numIterations) % worker
                ...
                if contains(o.ServerName, 'Whitneys-MacBook-Pro')
                ...
                % This try-catch block is designed to handle a specific issue where the hostname
                % may need to be appended with '.local' to be recognized, particularly on macOS systems.
                % The code first attempts to connect using the original hostname. If that fails,
                % it appends '.local' to the hostname and tries again. This ensures compatibility
                % across different systems and network configurations.
                % Initialize the connection string (replace with your actual connection string)                                               
                try
                    % Attempt to open the connection using the original hostname
                    fopen(o.connection);                    
                catch
                        % If the connection fails, append '.local' to the hostname and try again
                    disp('Failed to connect with original hostname. Retrying with .local suffix.');
                    
                    % Retrieve the original RemoteHost property
                    originalHost = get(o.connection, 'RemoteHost');
                    
                    % Append '.local' to the hostname
                    modifiedHost = [originalHost, '.local'];
                    
                    % Update the RemoteHost property
                    set(o.connection, 'RemoteHost', modifiedHost);
                    
                    try
                        % Attempt to open the connection using the modified hostname
                        fopen(o.connection);
                        disp('Connection successful with .local appended hostname.');
                    catch ME
                        % If the connection still fails, display the error information
        ...
        function delete( o )
            debug('Delete object');
            o.close();
            warning('on','instrument:udp:ClassToBeRemoved');
        end
